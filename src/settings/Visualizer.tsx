      ...
      {visualizers.map((section) => {
        ...
        return (
          <div className="settingsRow settingsInstance" key={instance} role="group" aria-labelledby={`instance-${instance}`}>
            ...
            <SettingSwitch isSelected={section.viz_mirror ?? false} onChange={(viz_mirror) => patch(instance, { viz_mirror })}>{section.id === "spectrum" ? "Flip vertically" : "Mirror frequency bands"}</SettingSwitch>
